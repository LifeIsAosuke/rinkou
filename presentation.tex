...
\begin{frame}
    ...
    \begin{alertblock}{確率の定義}
        シグモイド関数を適用することで、各クラスの確率を以下のように定義する
        \begin{eqnarray*}
...
\begin{frame}
    ...
    \begin{alertblock}{特徴量の設計}
        \begin{itemize}
            \item ロジスティック回帰では、入力のあらゆる性質を特徴量として利用できる
...
\begin{frame}
    ...
    \begin{alertblock}{特徴量の設計とスケーリング}
        \begin{itemize}
            ...
            \item 特徴量の相互作用\\複数の条件を組み合わせた複雑な特徴量。例えば、「現在の単語が"St."であり、かつ前の単語が固有名詞（大文字）である」場合、それは文末ではなく、「通り名（Street）」の略称である可能性が高い
        \end{itemize}
    \end{alertblock}
\end{frame}
...
\begin{frame}
    ...
    \begin{alertblock}{具体的な計算例}
        映画レビューの感情分析（スライドP9999）を例にとると、以下のようになる
        \centering
        \includegraphics[scale=0.55]{etc/4-3table.png}
        \includegraphics[scale=0.5]{etc/fig4.2.png}
    \end{alertblock}
