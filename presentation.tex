...
\begin{frame}
    ...
    \begin{alertblock}{2値分類の目標と数値化}
        \begin{itemize}
            \item 目標\\新しい入力観測値$x$に対して、特定のクラスに属するかどうか（0か1か）を判定すること
...
\begin{frame}
    ...
    \begin{exampleblock}{感情分析の例}
        正の重みは「クラス1である根拠」となり、負の重みは「クラス1でない（クラス0である）根拠」となる\\
...
\begin{frame}
    ...
    \begin{alertblock}{勾配降下法の目的と直感}
        \begin{itemize}
            ...
            \begin{equation*}
                \hat{\theta} = \underset{\theta}{\operatorname{argmin}} \frac{1}{m} \sum_{i=1}^m L_{CE} (f((x^{(i)};\hat{\hat{\theta}}), y^{(i)}))
            \end{equation*}
